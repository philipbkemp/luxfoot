...
function parseLeague(league) {
	...
	if ( league.matches ) {
		...
		hasTwo = false || (league.play_each && league.play_each === 3);
		matchesPanel.append( buildResultsTable(league.teams,league.matches,hasTwo,league.covid?true:false) );
		matchesPanel.append(
	...
	if ( league.series ) {
		...
		league.series.forEach(series=>{
			...
			addKeys(seriesKeys);
			$("#leagueTabs").append(buildTabButton("series_"+series.series,series.name,series.series===1,series.missing,true));
	...
	if ( league.playoffs ) {
		...
		if ( league.playoffs.league_promotion_playoff ) {
			$("#leagueTabs").append(buildTabButton("po_league_promotion_playoff","Promotion play-off"));
			poLeaguePromotionPlayoff = buildTabPanel("po_league_promotion_playoff");
		...
		if ( league.playoffs.league_downup || league.playoffs.league_updown ) {
			style = league.playoffs.league_downup ? "league_downup" : "league_updown";
			$("#leagueTabs").append(buildTabButton("po_"+style,getPlayoffName(style)));
			poThisPlayoff = buildTabPanel("po_"+style);
			poTheseMatches = $("<DIV></DIV>").addClass("list-group").addClass("mb-4");
			if ( league.playoffs[style].matches ) {
				league.playoffs[style].matches.forEach(m=>{
					poTheseMatches.append( drawMatch(m));
				});
				poThisPlayoff.append(poTheseMatches);
			}
			poThisPlayoff.append(
				buildStandings(
					league.playoffs[style].standings,
					league.playoffs[style].pts_win
						? league.playoffs[style].pts_win
						: 3,
					true
				)
			);
			$("#leagueTabContent").append(poThisPlayoff);
			removeKey("playoffs."+style);
		}

		if ( league.playoffs.league_relegation_playoff ) {
			$("#leagueTabs").append(buildTabButton("po_league_relegation_playoff","Relegation play-off"));
			poLeaguePromotionPlayoff = buildTabPanel("po_league_relegation_playoff");
			poLeaguePromotionPlayoffMatches = $("<DIV></DIV>").addClass("list-group").addClass("mb-4");
		...
		["final_round","relegation_a","relegation_b"].forEach(po=>{
			if ( league.playoffs[po] ) {
				...
				poThisRound = buildTabPanel("po_"+po);
				poThisRounddMatches = $("<DIV></DIV>").addClass("list-group").addClass("mb-4");
				poThisRound.append(
					buildStandings(
		...
		["a","b","c","d"].forEach(po=>{
			...
			if ( league.playoffs[poc] ) {
				...
				if ( league.playoffs[poc].matches ) {
					league.playoffs[poc].matches.forEach(m=>{
						poLeagueRelegationPlayoffMatches.append( drawMatch(m));
					});
					poLeagueRelegationPlayoff.append(poLeagueRelegationPlayoffMatches);
				}
				poLeagueRelegationPlayoff.append(
					buildStandings(
						league.playoffs[poc].standings,
						league.playoffs[poc].pts_win
							? league.playoffs[poc].pts_win
	...
	standings.forEach(s=>{
		...
		if ( s.relegated) {
			...
			if ( s.relegated_thrice ) {
				...
				removeKey("standings.relegated_thrice");
			}
