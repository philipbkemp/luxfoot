allTeams = {
    ...
    "FC Schengen": "SHGN", "RS Merl/Belair": "RSMB", "FC Red Black/Egalité 07": "RBE7", "US Esch-Alzette": "UESH", "FC The Belval Belvaux": "BLVL",
    "Union 05 Kayl-Tétange": "U5KT", "Red Star Merl-Belair": "RSMB", "FC Blo-Wäiss Izeg": "BWIT", "FC RB Egalité 07 Pfaffenthal-Weimerskirch": "RBE7",
...
document.querySelectorAll(".jss474").forEach(match=>{
    ...
    home = allTeams[home];
    away = match.querySelectorAll("b")[1].textContent;
    if ( ! allTeams[away] ) {
        allTeams[away] = prompt("CODE",away);
    }
    away = allTeams[away];
    //score = match.querySelectorAll("h3")[0].textContent.replace(" : ","-");
...
out = "[\n\t\t\t";
matches.forEach(m=>{out+=JSON.stringify(matches)+",\n\t\t\t";});
out += "]";
console.log(out);